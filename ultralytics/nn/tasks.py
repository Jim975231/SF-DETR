...
from ultralytics.nn.IRSTD_modules import *
...
class RTDETRDetectionModel(DetectionModel):
    ...
    def predict(self, x, profile=False, visualize=False, batch=None, augment=False):
        ...
        for m in self.model[:-1]:  # except the head part
            ...
            if visualize:
                feature_visualization(x, m.type, m.i, save_dir=visualize)
        head = self.model[-1]    #model[-1]即为模型配置文件的最后一层，RTDETRDecoder层
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, Focus,
                 BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, nn.ConvTranspose2d, DWConvTranspose2d, C3x, ConvNormLayer,RepC3,SFSCBlock,SRAIFI,CFARepC3):
            c1, c2 = ch[f], args[0]
            if c2 != nc:  # if c2 not equal to number of classes (i.e. for Classify() output)
                c2 = make_divisible(min(c2, max_channels) * width, 8)

            args = [c1, c2, *args[1:]]
            if m in (BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, C3x, RepC3,SFSCBlock,SRAIFI,CFARepC3):
