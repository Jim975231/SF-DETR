...
from ultralytics.utils.metrics import bbox_iou,irst_aware_bbox_loss, bbox_inner_iou, bbox_focaler_iou, bbox_mpdiou, bbox_inner_mpdiou, bbox_focaler_mpdiou, wasserstein_loss, kw_nwd_loss_norm,auto_tb_nwd_loss,asw_nwd_loss,WiseIouLoss
...
class DETRLoss(nn.Module):
    ...
    def __init__(self,
        ...
        if self.nwd_loss:
            self.iou_ratio = 0.5
            assert 0.0 <= self.iou_ratio <= 1.0, \
                f"iou_ratio must be in [0,1], got {self.iou_ratio}"
            print(f"[DETRLoss] NWD ENABLED, iou_ratio={self.iou_ratio}")
        ...
        if self.auto_nwd:
            ...
            assert 0.0 <= self.iou_ratio <= 1.0, \
                f"iou_ratio must be in [0,1], got {self.iou_ratio}"
            print(f"[DETRLoss] NWD-auto ENABLED, iou_ratio={self.iou_ratio}")

        #for KW-NWD-Norm
        self.use_kw_nwd = False
        if self.use_kw_nwd:
            self.iou_ratio = 0.4  # GIoU 的权重 (建议 0.4 GIoU + 0.6 KW-NWD)
            self.kw_alpha = 3.0  # 知识增强强度
            assert 0.0 <= self.iou_ratio <= 1.0, \
                f"iou_ratio must be in [0,1], got {self.iou_ratio}"
            print(f"[DETRLoss] kw_nwd ENABLED, iou_ratio={self.iou_ratio}")

        #for self.use_asw_nwd
        self.use_asw_nwd = False
        if self.use_asw_nwd:
            self.iou_ratio = 0.3
            print(f"[DETRLoss] ASW-NWD (Parameter-Free) ENABLED, iou_ratio={self.iou_ratio}")
        ...
        # 添加 IRST Loss 开关
        self.use_irst_loss = False
        if self.use_irst_loss:
            print(f"🚀 [DETRLoss] IRST-Aware BBox Loss ENABLED ")
    ...
    def _get_loss_class(self, pred_scores, targets, gt_scores, num_gts, postfix=''):
        ...
        if self.sl or self.emasl:
            ...
            loss_cls = nn.BCEWithLogitsLoss(reduction='none')(pred_scores, gt_scores).mean(1).sum()  # YOLO CLS loss  默认的分类损失
    ...
    def _get_loss_bbox(self, pred_bboxes, gt_bboxes, postfix=''):
        ...
        if len(gt_bboxes) == 0:
            ...
            return loss   #如果没有任何真值框，直接返回两个零张量作为损失

        loss[name_bbox] = self.loss_gain['bbox'] * F.l1_loss(pred_bboxes, gt_bboxes, reduction='sum') / len(gt_bboxes)  #计算L1 Loss损失
        ...
        if self.nwd_loss:
            ...
            nwd = auto_tb_nwd_loss(pred_bboxes, gt_bboxes)
            loss[name_giou] = self.iou_ratio * (loss[name_giou].sum() / len(gt_bboxes)) + (1.0 - self.iou_ratio) * ((1.0 - nwd).sum() / len(gt_bboxes))
        elif self.use_kw_nwd:
            # 直接传入 pred_bboxes 和 gt_bboxes (它们已经是归一化的，不需要 *640)
            # kw_nwd_loss_norm 内部已经处理了归一化参数 base_c=0.02
            loss_kw = kw_nwd_loss_norm(pred_bboxes, gt_bboxes, alpha=self.kw_alpha, base_c=0.02)

            # 混合 Loss: Ratio * GIoU + (1-Ratio) * KW-NWD
            # 注意：这里需要先 sum 再除以 len，保持与 RT-DETR 原版逻辑一致
            term_giou = loss[name_giou].sum() / len(gt_bboxes)
            term_kw = loss_kw.sum() / len(gt_bboxes)

            loss[name_giou] = self.iou_ratio * term_giou + (1.0 - self.iou_ratio) * term_kw
        elif self.use_asw_nwd:
            # --- ASW-NWD 逻辑 ---
            # 1. 调用无参函数 (base_c=0.02 适配归一化)
            loss_asw = asw_nwd_loss(pred_bboxes, gt_bboxes, base_c=0.02)

            # 2. 聚合 Loss (Sum / Len)
            term_giou = loss[name_giou].sum() / len(gt_bboxes)
            term_asw = loss_asw.sum() / len(gt_bboxes)

            # 3. 【补上这一行】混合并赋值
            loss[name_giou] = self.iou_ratio * term_giou + (1.0 - self.iou_ratio) * term_asw
        elif self.use_irst_loss:
            # 1) 计算 GIoU 分值（用于 IRST 损失内部的接力混合）
            giou_vals = bbox_iou(pred_bboxes, gt_bboxes, xywh=True, GIoU=True)  # [N]

            # 2) 调用 IRST 损失（替代原来的 GIoU 损失）
            loss_irst = irst_aware_bbox_loss(pred_bboxes, gt_bboxes, giou_vals)

            # 3) 将 IRST 综合损失填入 loss_giou，替代原来的 GIoU
            loss[name_giou] = loss_irst.mean() * 2.0
        else:
