...
def irst_loss_set_epoch(epoch: int):
    """
    在每个 epoch 结束后调用，打印上一个 epoch 的统计并重置累积。
    建议在训练循环的每个 epoch 末尾调用：irst_loss_set_epoch(epoch)
    """
    global _irst_loss_state
    prev_epoch = _irst_loss_state['epoch']
    accum = _irst_loss_state['accum']

    # 打印上一个 epoch 的统计（跳过初始状态 -1）
    if prev_epoch >= 0 and len(accum['total']) > 0:
        def _stat(key):
            t = torch.stack(accum[key])
            return t.mean().item(), t.max().item(), t.min().item()

        print(f"\n{'='*65}")
        print(f"[IRST Loss] Epoch {prev_epoch} 统计  "
              f"（共 {len(accum['total'])} 次迭代）")
        print(f"{'='*65}")

        mean_scale, max_scale, min_scale = _stat('gt_scales')
        mean_tau = torch.stack(accum['tau']).mean().item()
        print(f"  [尺度]  gt_scales: min={min_scale:.4f}  "
              f"max={max_scale:.4f}  mean={mean_scale:.4f}  "
              f"tau(mean)={mean_tau:.4f}")

        mean_wc, max_wc, _ = _stat('W_center')
        mean_ws, max_ws, _ = _stat('W_size')
        print(f"  [权重]  W_center: mean={mean_wc:.3f}  max={max_wc:.3f}  |  "
              f"W_size: mean={mean_ws:.3f}  max={max_ws:.3f}")

        mean_lc, max_lc, _ = _stat('loss_center')
        flag_lc = '🚨' if mean_lc > 2.0 else '✅'
        print(f"  [损失]  loss_center:    mean={mean_lc:.4f}  max={max_lc:.4f}  {flag_lc}")

        mean_ln, max_ln, _ = _stat('loss_nwd')
        print(f"  [损失]  loss_nwd:       mean={mean_ln:.4f}  max={max_ln:.4f}  ✅ 正常范围[0,1]")

        mean_lg, max_lg, _ = _stat('loss_giou')
        print(f"  [损失]  loss_giou:      mean={mean_lg:.4f}  max={max_lg:.4f}  ✅ 正常范围[0,2]")

        mean_geo, max_geo, _ = _stat('loss_geometry')
        print(f"  [损失]  loss_geometry:  mean={mean_geo:.4f}  max={max_geo:.4f}")

        mean_ls, max_ls, _ = _stat('loss_shape')
        flag_ls = '🚨' if mean_ls > 2.0 else '✅'
        print(f"  [损失]  loss_shape:     mean={mean_ls:.4f}  max={max_ls:.4f}  {flag_ls}")

        mean_tot, max_tot, _ = _stat('total')
        flag_tot = '🚨 过大！超出GIoU正常量级' if mean_tot > 4.0 else '✅ 量级正常'
        print(f"  [损失]  total:          mean={mean_tot:.4f}  max={max_tot:.4f}  {flag_tot}")

        if mean_lc > 2.0:
            print(f"  ⚠️  loss_center 均值 >2.0，可考虑降低学习率或增加梯度裁剪")
        if mean_ls > 2.0:
            print(f"  ⚠️  loss_shape 均值 >2.0，当前仅用宽高比，可检查目标形状是否非圆形")
        if mean_tot > 4.0:
            print(f"  ⚠️  total_loss 均值 >4.0，可能梯度爆炸或回归权重过高")
        print(f"{'='*65}\n")

    # 重置累积并更新当前 epoch
    _irst_loss_state['epoch'] = epoch
    for key in accum:
        accum[key].clear()

def irst_aware_bbox_loss(pred, target, giou_val, eps=1e-7):
    """
    红外小目标自适应鲁棒回归损失（完全替代 GIoU 项）
    giou_val: 外部传入的 GIoU 分值 (0~1)，形状 [N]
    """
    # 1. 自动计算 tau (适配不同分辨率/数据集)
    with torch.no_grad():
        gt_scales = torch.sqrt(target[:, 2] * target[:, 3])
        tau = gt_scales.median().clamp(min=0.01)

    # 2. 权重分配 (W_center + W_size = 2.0)
    W_center = 1.0 + torch.exp(-gt_scales / tau)
    W_size   = 2.0 - W_center

    # 3. 中心点精对齐 (尺度归一化)
    dist2 = (pred[:, 0] - target[:, 0]) ** 2 + (pred[:, 1] - target[:, 1]) ** 2
    loss_center = W_center * (dist2 / (gt_scales ** 2 + eps))

    # 4. 几何接力回归 (NWD + GIoU 自适应混合)
    # NWD：负责小目标无重叠时的强力拉近
    wh_dist2 = ((pred[:, 2] - target[:, 2]) ** 2 + (pred[:, 3] - target[:, 3]) ** 2) / 4.0
    loss_nwd = 1.0 - torch.exp(-torch.sqrt(dist2 + wh_dist2 + eps) / 0.02)

    # GIoU：负责大目标的重叠面积优化
    loss_giou = 1.0 - giou_val

    # 自适应接力：小目标信任 NWD，大目标引入更多 GIoU
    loss_geometry = W_center * loss_nwd + W_size * loss_giou

    # 5. 形状约束
    log_ar_loss = (torch.log(pred[:, 2].clamp(min=eps))
                   - torch.log(target[:, 2].clamp(min=eps))
                   - torch.log(pred[:, 3].clamp(min=eps))
                   + torch.log(target[:, 3].clamp(min=eps))).abs()
    loss_shape = W_center * log_ar_loss

    return loss_center + loss_geometry + loss_shape        # [N]
...
def auto_tb_nwd_loss(pred, target, eps=1e-7):
    """
    Auto-TB-NWD (Parameter-Free Thermal-Blooming NWD)

    【核心特性】
    1. 无额外参数：完全去除 alpha, base_scale 等人为超参。
    2. 自适应物理基准：利用 NWD 标准常数 C=12.8 作为物理尺度的"锚点"。
    3. 鲁棒性：C_dynamic 的范围被自动限制在 [12.8, 25.6] 之间，既防止梯度消失，又防止发散。

    Args:
        pred: [x, y, w, h] (N, 4), 绝对像素坐标
        target: [x, y, w, h] (N, 4), 绝对像素坐标
    """
    # NWD 的标准常数 (这也是红外小目标检测的通用经验值)
    # 不属于"额外"参数，而是算法自带的固有属性
    C = 12.8

    b1_x, b1_y, b1_w, b1_h = pred.unbind(-1)
    b2_x, b2_y, b2_w, b2_h = target.unbind(-1)

    # 1. 计算几何 Wasserstein 距离 (保持不变)
    center_dis = (b1_x - b2_x).pow(2) + (b1_y - b2_y).pow(2)
    wh_dis = ((b1_w - b2_w) / 2).pow(2) + ((b1_h - b2_h) / 2).pow(2)

    # 2. 【无参自适应模块】
    # 计算目标自身的物理尺度
    target_scale = torch.sqrt(b2_w * b2_h).clamp(min=1.0)

    # 动态调整逻辑：
    # 利用 C 作为尺度的衡量标准。
    # 如果 target_scale < C (极小目标)，指数项趋近 1，C_dyn 趋近 2*C (更宽容)
    # 如果 target_scale > C (大目标)，指数项趋近 0，C_dyn 趋近 C (标准精度)
    c_dynamic = C * (1 + torch.exp(-target_scale / C))

    # 3. 各向异性归一化 (Anisotropic Strategy)
    # 同样采用"严守中心，放宽形状"的策略，但无需额外参数
    # 中心点：始终用标准 C 约束，保证 mAP75
    # 形状：用 c_dynamic 约束，容忍热晕带来的边缘模糊
    metric = center_dis / (C ** 2 + eps) + wh_dis / (c_dynamic ** 2 + eps)

    # 4. 计算 Loss
    nwd = torch.exp(-torch.sqrt(metric + eps))

    return nwd


def kw_nwd_loss_norm(pred, target, alpha=2.0, base_c=0.02, ref_scale=0.05, eps=1e-7):
    """
    KW-NWD-Norm: Knowledge-Weighted NWD for Normalized Coordinates.

    专为 RT-DETR/YOLO 设计的归一化版本。
    不需要乘以图像尺寸，直接对 [0,1] 的 pred/target 计算 Loss。

    Args:
        pred: [cx, cy, w, h] (N, 4), 归一化坐标 [0-1]
        target: [cx, cy, w, h] (N, 4), 归一化坐标 [0-1]
        alpha (float): 知识权重强度。值越大，对小目标的 Loss 惩罚越重。
                       建议范围 [2.0, 4.0]。
        base_c (float): 归一化后的 NWD 常数 C。
                        原始 C=12.8px, 假设 img_sz=640, 则 base_c = 12.8/640 ≈ 0.02。
        ref_scale (float): 定义"小目标"的归一化参考尺度。
                           假设小目标 < 32px, 则 ref_scale = 32/640 = 0.05。

    Returns:
        loss: 加权后的 NWD Loss 张量
    """
    # 1. 解包坐标 (Center X, Center Y, Width, Height)
    b1_x, b1_y, b1_w, b1_h = pred.unbind(-1)
    b2_x, b2_y, b2_w, b2_h = target.unbind(-1)

    # 2. 计算 Wasserstein 距离 (基于归一化数值)
    # 由于坐标是 0-1，这里的数值会非常小 (e.g., 1e-5)
    center_dis = (b1_x - b2_x).pow(2) + (b1_y - b2_y).pow(2)
    wh_dis = ((b1_w - b2_w) / 2).pow(2) + ((b1_h - b2_h) / 2).pow(2)

    # 3. 计算标准 NWD
    # 使用归一化的 base_c (0.02) 作为分母，抵消分子的微小数值，恢复正常的梯度流
    metric = (center_dis + wh_dis) / (base_c ** 2 + eps)
    nwd_metric = torch.exp(-torch.sqrt(metric))
    loss_nwd = 1 - nwd_metric

    # 4. 【知识增强模块】尺度逆注意力权重 (Scale-Inverse Attention)
    # 计算目标的归一化尺度 (sqrt(w*h))
    tgt_scale = torch.sqrt(b2_w * b2_h).detach()  # detach 防止梯度反传给权重

    # 权重公式：W = 1 + alpha * exp(-scale / ref)
    # 物理意义：
    # - 当目标极小 (scale -> 0) 时，Weight -> 1 + alpha (重加权)
    # - 当目标较大 (scale >> ref) 时，Weight -> 1 (退化为普通 NWD)
    scale_weight = 1 + alpha * torch.exp(-tgt_scale / ref_scale)

    # 5. 返回加权 Loss
    return loss_nwd * scale_weight
...
def asw_nwd_loss(pred, target, base_c=0.02, eps=1e-7):
    """
    ASW-NWD (Adaptive Scale Weight NWD) - 修正归一化版

    Args:
        pred: [cx, cy, w, h] (N, 4), 归一化坐标 [0-1]
        target: [cx, cy, w, h] (N, 4), 归一化坐标 [0-1]
        base_c (float): NWD 常数归一化值。
                        对应原始像素常数 C=12.8。
                        在 640x640 图像下，12.8 / 640 = 0.02。
                        务必保持默认值 0.02 以适配 RT-DETR。
        eps (float): 数值稳定性常量。
    """
    b1_x, b1_y, b1_w, b1_h = pred.unbind(-1)
    b2_x, b2_y, b2_w, b2_h = target.unbind(-1)
    ...
    # 度量值: 距离 / C^2
    # 这里 base_c=0.02，平方后为 0.0004，正好抵消分子的微小数值
    metric = (center_dis + wh_dis) / (base_c ** 2 + eps)
    nwd_metric = torch.exp(-torch.sqrt(metric))
    loss_nwd = 1 - nwd_metric

    # 2. 计算无参尺度权重 (Scale Weight)
    # 目标归一化尺度 (sqrt(w*h))
    tgt_scale = torch.sqrt(b2_w * b2_h).detach()

    # 权重函数: W = 1 + exp(-scale / C)
    # 同样使用 base_c=0.02 作为尺度的衡量基准
    scale_weight = 1 + torch.exp(-tgt_scale / base_c)
